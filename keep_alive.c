	...
	if (name != NULL) {
		...
		if (watchdog_fd == -1) {
			...
			if (ioctl(watchdog_fd, WDIOC_GETSUPPORT, &ident) < 0) {
				...
				if (strcmp("IPMI timer", ident.identity) != 0) {
                    			log_message(LOG_ERR, "This is not an IPMI watchdog. Exiting...");
                    			close_watchdog(); // Close the watchdog before exiting
                    			system("systemctl stop watchdog.service");
					exit(EXIT_FAILURE);  // Exit with failure if it's not the i6300ESB timer
                		}
			}
		}
	}
